...
#define ADMIN_LVL ADMIN_RESERVATION 		// flag for admin
...
public plugin_init()
{
	register_plugin("[Jb] Extra: Dual Infinity", "1.0", "Crock / =) (Poprogun4ik) / LARS-DAY[BR]EAKER / fayting")
	...
	cvar_clip_infi = register_cvar("jb_infi_clip", "30")
	cvar_infi_ammo = register_cvar("jb_infi_ammo", "200")
	cvar_dmg_infi = register_cvar("jb_infi_dmg", "1.13")
	cvar_recoil_infi = register_cvar("jb_infi_recoil", "0.8")
	cvar_spd_infi = register_cvar("jb_infi_spd", "1.2")
	cvar_spd_mode_infi = register_cvar("jb_infi_mode_spd", "0.95")
	
	register_clcmd("infinity", "give_infinity")
...
public give_infinity(id, itemid)
{
	if(!(get_user_flags(id) & ADMIN_LVL)) 
		return
