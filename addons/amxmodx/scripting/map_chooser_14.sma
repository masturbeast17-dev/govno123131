...
//#define ADMIN_DUAL_VOTE (ADMIN_BAN)
...
/* Словарь плагина */
#define MSG_NOMINATE_BLOCKED 		"^1[^4карта^1] ^4Данная карта была недавно сыграна!"
#define MSG_NOMINATE_DISABLE 		"^1[^4карта^1] ^4Номинация недоступна!"
#define MSG_NOMINATE_MAX_MAP_PL 	"^1[^4карта^1] ^4Вы уже номинировали максимум карт ^3[%d]^4!"
#define MSG_NOMINATE_MAX_MAP_ALL 	"^1[^4карта^1] ^4Уже номинировано максимум карт ^3[%d]^4!"
#define MSG_NOMINATE_MAP_NOMINATED	"^1[^4карта^1] ^4Данная карта уже номинирована!"
#define MSG_NOMINATE_PL_NOMINATEMAP	"^1[^4карта^1] ^3%s ^4номинировал карту ^3%s"

#define MSG_RTV_BLOCKED0		"^1[^4карта^1] ^4Досрочная смена карты будет доступна менее, чем через ^3минуту^4!"
#define MSG_RTV_BLOCKED			"^1[^4карта^1] ^4Досрочная смена карты будет доступна через ^3%d ^4мин!"
#define MSG_RTV_PL_VOTED		"^1[^4карта^1] ^4Вы уже голосовали! Осталось ^3%d ^4голосов"
#define MSG_RTV_PL_VOTE			"^1[^4карта^1] ^3%s ^4проголосовал за смену карты. Осталось ^3%d ^4голосов"
#define MSG_RTV_VOTE_START		"^1[^4карта^1] ^4Все голоса за досрочную смену карты набраны. ^3Последний ^4раунд"
#define MSG_ADMIN_RTV			"^1[^4карта^1] ^4Администратор ^3%s ^4запустил досрочную смену карты. ^3Последний ^4раунд"

#define MSG_CMD_FFIRE			"^1[^4карта^1] ^4На сервере огонь по своим  ^3%s"
#define MSG_CMD_THETIME			"^1[^4карта^1] ^4Текущее время ^3%s"
#define MSG_CMD_NEXTMAP			"^1[^4карта^1] ^4Следующая карта еще ^3не определена ^1:("
#define MSG_CMD_TIMELEFT0		"^1[^4карта^1] ^4Карта ^3не ограничена ^4по времени"
#define MSG_CMD_TIMELEFT		"^1[^4карта^1] ^4До конца карты осталось ^3%d ^4мин ^3%02d ^4сек"
#define MSG_CMD_TIMELEFT_LAST_RND	"^1[^4карта^1] ^4Карта окончена. ^3Последний ^4раунд!"

#define MSG_TIMELEFT_ON_ROUNDSTART	"^1[^4карта^1] ^4До конца карты осталось ^3%d ^4мин ^3%02d ^4сек%s"
#define MSG_NOTIMELIMIT_ON_ROUNDSTART	"^1[^4карта^1] ^4Карта ^3не ограничена ^4по времени"
#define MSG_TIMELEFT_ADD_LASTRND	". ^3Последний ^4раунд!"

#define MSG_TIME_TO_VOTE		"До голосования осталось %d сек!"

#define MSG_VOTEMAP_PL_EXT		"^1[^4карта^1] ^4Игрок ^3%s ^4выбрал ^3продление карты"
#define MSG_VOTEMAP_PL_MAP		"^1[^4карта^1] ^4Игрок ^3%s ^4выбрал карту ^3%s"

#define MSG_VOTE_END_NOVOTES		"^1[^4карта^1] ^4Никто ^3не проголосовал! ^4Cлучайная карта ^3%s"
#define MSG_VOTE_END_EXTENDED		"^1[^4карта^1] ^4Голосование ^3завершено! ^4Карта продлена на ^3%d ^4минут"
#define MSG_VOTE_END_NEXTMAP		"^1[^4карта^1] ^4Голосование ^3завершено! ^4Cледующая карта ^3%s"

#define MSG_NIGHT_BLOCK_CMD		"^1[^4карта^1] ^4Данная команда ^3заблокирована ^4в ^1ночном ^4режиме!"
...
public plugin_init()
{
	register_plugin("Advanced MapChooser", "1.4.1", "minibam");
	...
	register_concmd("amx_rtv",	"ConCmdAdminRockTheVote", ADMIN_BAN);
#endif
...
public ConCmdAdminRockTheVote(id, bitAccess)
{
	...
	if(id)
	{
		...
		if(~flags & ADMIN_BAN)
		{
			...
			{
				client_print_color(id, print_team_default, "^1[^4карта^1] ^3Досрочное голосование ^4будет доступно через ^3%d ^4мин", ADMIN_RTV_TIME - time);
				return PLUGIN_HANDLED;
			}
		}
	}
